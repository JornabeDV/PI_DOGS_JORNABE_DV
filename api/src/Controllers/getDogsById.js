...
const {Dog, Temperaments} = require("../db"); //Se importa el modelo DOG, que es el modelo Sequelize relacionado con los perros en la base de datos.-
const {getDogsMap} = require ("./getAllDogs.js"); // e importa la función getDogsMap desde el módulo getAllDogs.js, lo que indica que esta función se utiliza para formatear los datos del perro.-
...
const getDogsById = async (id, source) => {//Parámetros vienen del Handler.-
    ...
    }) //Si la fuente no es "api" (lo que sugiere que se buscará en la base de datos local), se utiliza el método findByPk del modelo DOG de Sequelize para encontrar el perro por su Primary Key (PK), que en este caso es el ID.-
